...
class LRUBufferManager(BufferManager):
    ...
    # Un solo IOStats por StorageEngine, nadie más lleva su propio contador
    def stats(self) -> IOStats:
        return self._storage.stats()

    # Pasa la asignación de páginas al StorageEngine, el buffer no reserva espacio
    def allocate_page(self, file_id: str) -> int:
        return self._storage.allocate_page(file_id)

    # Primero se eligen todas las víctimas, recién después se escriben juntas
    def _make_room(self, protect: tuple[str, int] | None = None) -> None:
        ...
        if len(victims) < overflow:
            raise RuntimeError("el buffer está lleno y todas sus páginas están fijadas")
